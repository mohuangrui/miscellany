...
//*
//* This script add a computed column data to each case.
//*
//*                              By Huangrui Mo
//*
//****************************************************************************//
...
Column_List=[2:1:$]; //* the list of target columns No.
...
write(Data_Out_FID,"* New Column Added *",'(A)');
write(Data_Out_FID,"*******************************************************",'(A)');
...
for n = 1 : Case.Amount
    ...
    //* set new collumn data by transforming to spatial coordinates
    lMin=0;
    lMax=1;

    sampleN=Row.Amount;
    ds=(lMax-lMin)/(sampleN-1);

    New_Col=lMin + (Seq-1)*ds;

    //* get result data
    Case.Rlt(1)=[New_Col,Case.Data(1)];
