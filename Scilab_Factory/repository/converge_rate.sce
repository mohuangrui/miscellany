...
//*
//* This script reads in specified data and calculate convergence rate based
//* on each column data.
//*
//*                              By Huangrui Mo
//*
...
//*
//****************************************************************************
//* convergence rate computation type
compute_type=0; //* 0: absolute error based; 1: relative error based (require fixed grid ratio)
...
write(Data_Out_FID,"* Convergence rate *",'(A)');
write(Data_Out_FID,"*******************************************************",'(A)');
...
for n = 1 : Case.Amount
    ...
    p = [];//* observed convergence rate
    if compute_type == 0 then
        for n = 2:1:Row.Amount
            r = Case.Data(1)(n,1) / Case.Data(1)(n-1,1);//* grid refinement ratio
            em = Case.Data(1)(n-1,2:$); //* e(n-1)
            en = Case.Data(1)(n,2:$); //* e(n)
            p(n,:)=log2(em ./ en) / log2(r);
        end
    else
        for n = 3:1:Row.Amount
            r = Case.Data(1)(n,1) / Case.Data(1)(n-1,1);//* grid refinement ratio
            em = abs(Case.Data(1)(n-2,2:$) - Case.Data(1)(n-1,2:$)); //* e(n-1)
            en = abs(Case.Data(1)(n-1,2:$) - Case.Data(1)(n,2:$)); //* e(n)
            p(n,:)=log2(em ./ en) / log2(r);
        end
    end
    Case.Rlt(1)=p;
