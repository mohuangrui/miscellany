...
//* Comments: this script stores all the data within the specifid range, so it 
//* can apply to a wide range of data processing, the data between cases can
//* manipulate with each other to generate new data sets. however, this feature
//* makes less efficiency and requires a lot of memory.
...
for n = 1 : Case.Amount
    ...
    //* extract the specified range of data from LoadData(1) to Case.Data(n)
    Case.Data(n)=LoadData(1)(Row.Start : Row.End,Col.List);
end
CommentLine=read(Data_In_FID, 1, 1,'(A)');
